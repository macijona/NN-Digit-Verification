...
number_of_layers = 4
width_of_layers = 50  # only matters if number of layers > 1
...
def main():
  ...
  # Evaluate on Test
  ################################
  evaluate_test(net, X_test)

def evaluate_test(model, test):
  logits = model.forward(test)
  probs = np.argmax(logits,axis=1)[:,np.newaxis]
  with open('predictions.csv', 'w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(["id","digit"])
    for i in range(len(probs)):
      writer.writerow([i, int(probs[i]) ])
...
class LinearLayer:
  ...
  def backward(self, grad):
    self.grad_weights = self.input.T@grad # X^T * DL/DZ
    self.grad_bias = np.sum(grad) # DL/DZ * 1^T
    return (grad@self.weights.T) # DL/DZ * W^T
